...
import { FaAddressBook, FaCommentAlt, FaDocker, FaUserCircle } from "react-icons/fa";
...
export function Menu({ setToggle, toggle }: { setToggle: React.Dispatch<React.SetStateAction<boolean>>; toggle: boolean }) {
  ...
  return (
    <>
      {/** 사이드 바 */}
      <div className={`flex flex-col lg:shadow-md lg:bg-white lg:gap-6 lg:justify-between lg:p-5 lg:w-80 sm:w-full sm:fixed sm:top-0 sm:left-0 sm:right-0 `}>
        {/** 사이드 바 타이틀 영역 */}
        ...
        {/* 버튼 */}
        <div className={`lg:flex lg:gap-5 lg:justify-center lg:pb-3 sm:hidden`}>
          <div
          ...
          >
            <Link to={`/admin/management/${id}/profile`}>
              <FaUserCircle />
            </Link>
          </div>
          <div
