...
export default function AuthView() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<{ loginId: string; password: string }>({
    mode: "onBlur",
  });

  const signInHandler = (data: { loginId: string; password: string }) => {
    signIn(data);
  };

  return (
    /** Div - All BackGround */
    <form className={"bg-[#f8fafb] lg:min-h-screen flex items-center justify-center"} onSubmit={handleSubmit(signInHandler)}>
      {/** Div - Login Box */}
